...
We use a collection-priority library for our ebooks and a series-priority library for our comics. We find that the collection-priority library works well for ebooks, as we can group them all by author, regardless of how we organize the underlying filesystem, and the series-priority library works well for comics, as we can group them by major arcs.
