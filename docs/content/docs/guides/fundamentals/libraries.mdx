...
A **Library** is where a run lives on disk — point Longbox at a folder and it files everything inside. A couple of concepts shape how it reads that shelf:
...
As long as a directory is accessible by Longbox, and abides by the above specification, you can create a library with it. This allows for convenient configurations like network drives, etc.

## » How the shelf gets sorted (supported patterns)

There are way too many varying organizational preferences for Longbox to support all of them. Instead, Longbox supports two different patterns for organizing your library:
...
<Callout>
	**FROM THE LONGBOX** — We're **very** actively trying to figure out better names for these
	patterns. Please reach out if you have any suggestions!
</Callout>
...
A few examples to help illustrate the difference between the two library patterns and how Longbox will interpret their differing structures.
...
When Longbox scans this library as a collection-priority library, it will create 3 series:

1. _Shannon, Samantha_
2. _Sanderson, Brandon_
3. _J.R.R. Tolkien_.

When Longbox scans this library as a series-priority library, it will create 4 series:
...
Below is a visualization of this library in Longbox, using a file tree for simplicity:
...
When Longbox scans this library as a series-priority library, it will create 2 series:
...
Notice how we started mixing stand alone novels and book series, grouping the book series by folders, for each author.
...
When Longbox scans this library as a series-priority library, it will create 7 series:
...
Personally, we use a collection-priority library for our ebooks and a series-priority library for our comics. We find that the collection-priority library works well for ebooks, as we can group them all by author, regardless of how we organize the underlying filesystem, and the series-priority library works well for comics, as we can group them by major arcs.

#### Alternative Options

Unfortunately, Longbox does not support any other patterns, as there are simply too many ways to organize a library to support them all. If you have a library that doesn't quite fit either of these patterns, or you just prefer a different organization method, you can always use the [File Explorer](/docs/guides/features/file-explorer) to navigate your library. This is akin to using a native file explorer to navigate your filesystem.
...
<Callout>
	**PULL LIST** — This section is not feature complete and is subject to change in the future.
</Callout>
...
> Deleting a library will remove all of the series, books, and other entities associated with the books (e.g., read progress, reading lists, etc) contained within it. Your actual files will not be deleted, but they will no longer exist in Longbox.
>
