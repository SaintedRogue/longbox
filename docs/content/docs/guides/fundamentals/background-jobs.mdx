...
Longbox runs certain operations as background jobs so the server remains responsive. Background jobs are usually long-running operations that may involve heavier CPU or IO work, such as scanning a library.
