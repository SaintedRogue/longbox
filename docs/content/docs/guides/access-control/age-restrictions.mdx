...
<Callout>
	**HOUSE RULES** — Please ensure you properly test any configured age restrictions to ensure they
	work as expected with your library **before** you give the restricted user access to their
	account.
</Callout>

Age restrictions are your shelf's content rating, set per user to decide whether a given book is theirs to open. For more information on users and user management, see the dedicated [users](/docs/guides/access-control/users) page.

### Supported age restrictions representations

The way age restrictions can be represented vary, but Longbox aims to support as many as possible and convert it to a common format to be used internally.

Longbox treats the age restriction as a **minimum age**. This means that if a user has an age restriction set to `13`,and a given book is rated at `17`, the user will not be able to access the book.
...
### Where does Longbox get the age restrictions from?

The age restriction is located directly within the metadata for a book itself, or on a book's series' metadata. Longbox will attempt to parse this value from the metadata and store it internally as a number (see above).

This means that **without metadata, Longbox cannot determine whether or not a book is age-allowed**. There are fallback options described below.

#### EPUB

Longbox will attempt to parse one of the following from an EPUB file's metadata:
...
### How does Longbox determine whether or not a book is age-allowed?

If a book or a book's series has an age restriction set, Longbox will use that age restriction to determine whether or not a user can access it. The comparison done internally is `less than or equal to X number`, meaning that if a user has an age restriction set to `13` and a book is rated to `17`, the user will not be able to access the book. If a user has an age restriction set to `17` and a book is rated to `13`, the user will be able to access the book. In other words, **the age restriction set on the user must be greater than or equal to the age restriction number set on the book or series in order to have access**.

If a book doesn't have an age restriction set, but a user with an age restriction attempts to access it, Longbox will use the following logic to determine whether or not the book is allowed:
