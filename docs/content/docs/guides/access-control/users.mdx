...
Every collection needs someone minding the shelves. Longbox has two user account types:
...
An 'unclaimed' Longbox server, or a server that has no user with the `server owner` role, will prompt for an initialization step, and will automatically assign the first registered user the **Server Owner** role.

## Management

<Callout title="Permission required">
	**HOUSE RULES** — These functionalities are gated behind the `user:manage` user permission. To
...
Longbox currently does not enforce any password complexity requirements. This can change if there is enough demand for it. In general, Longbox follows a fairly standard security model:
...
2. Use the embedded CLI in the Longbox server to unlock the user account. See the [CLI](/docs/developer/cli) guide for more information. In general, the command will look like this:

   ```bash copy
   ./longbox account unlock --username <username>
   ```

   Similarly, you can lock a user account using the following command:

   ```bash copy
    ./longbox account lock --username <username>
   ```
...
If a user has forgotten their password, you will have to use the embedded CLI in the Longbox server to reset the user's password. See the [CLI](/docs/developer/cli) guide for more information. In general, the command will look like this:

```bash copy
./longbox account reset-password --username <username>
```
