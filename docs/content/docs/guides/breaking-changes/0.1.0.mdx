...
Longbox has migrated from using Prisma to SeaORM for its database layer and from REST to GraphQL for its API. This was effectively a ground-up rewrite of a large portion of the entire codebase, and as such, there are several breaking changes that users should be aware of before upgrading to version 0.1.0 or later.

## Incompatible Database

I tried to minimize the number of breaking changes in the database, but there were some changes that were unavoidable. As a result, the database itself is not compatible with previous versions of Longbox.
...
This section outlines the steps needed to migrate your existing Longbox instance to version 0.1.0 or later. Please note that these steps only apply if you are upgrading from a version prior to 0.1.0.

I should also note that if you don't have a complicated setup, and you don't care about losing things like reading history, it might just be easier to start fresh with a new Longbox instance. The migration process is primarily intended for users who have a significant amount of data and configurations that they wish to retain.
...
Before proceeding with the migration, ensure you have a complete backup of your Longbox database. This is crucial in case anything goes wrong during the migration process.
</Step>

<Step>
### Create a separate Longbox instance

I absolutely do not recommend updating your existing Longbox instance in place. Please do not do that. This process should be in total isolation from your existing instance to avoid any potential data loss. We will bring over your existing configs and database in the next steps.

You could do something like the following:

```bash
/stump-old/      # Your existing Longbox instance
	├── docker-compose.yml
	└── config/
	    └── stump.db
/stump-new/      # Your new Longbox instance
	├── docker-compose.yml
...
- Make sure to fully stop your existing Longbox instance before starting the new one (even though they are isolated)
</Step>

<Step>
### Start the new Longbox instance

Start the new Longbox instance to allow it to create all the fresh configs and database files. Once you see the startup messages in the logs, fully stop the instance.

Be sure your old Longbox instance is fully stopped as well. At this point, you can copy over your old instance's `stump.db` file into the new instance's config directory as `stump-legacy.db` (or any other name you prefer, just not `stump.db`). You may also mount is separately if preferred.
</Step>
...
The updated Longbox instance includes a CLI tool to help with the migration process. Before continuing, please ensure the following:

- The new Longbox instance has been started at least once, has not been configured, and is fully stopped. There should be a new database created with no data in any of the tables
- The old Longbox instance is fully stopped
- The old Longbox database is accessible (if running Docker, ensure both database files are mounted properly or you copied it over accordingly)
