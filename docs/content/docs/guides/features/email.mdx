...
<Callout title="Permission required">
	**HOUSE RULES** — Sending emails is gated behind the `email:send` user permission at a minimum. To
	learn more about permissions, see the [permissions](/docs/guides/access-control/permissions) guide.
</Callout>

Longbox can drop a book straight into your inbox — or better yet, your e-reader's. This basic email functionality was primarily developed to support email-to-ereader integrations, however there are loose plans for expanding it in the future.

## Emailers

An emailer is essentially just a named configuration object that tells Longbox how to send emails from a specific email account. Longbox doesn't have its own email server or client, so it relies on SMTP to send emails. This means that you'll need to provide the necessary connection and auth details to send emails.
...
<Callout>
	**MINT CONDITION** — The password field is encrypted at rest and not visible after creation.
	Longbox will decrypt the password when sending emails. This is a security measure to prevent
	unauthorized access to your email server. You should still take care to secure your Longbox
	instance.
</Callout>
...
Longbox will log each email sent, however the logs are not currently exposed in the UI. This is a planned feature, but for now you can view the logs directly in the database with your preferred SQL client.
...
<Callout type="idea">
	**FROM THE LONGBOX** — If you have any ideas for expanding the email functionalities, feel free to
	open an issue on [GitHub](https://github.com/SaintedRogue/longbox/issues)
</Callout>
