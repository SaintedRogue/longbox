...
<Callout type="warn">
	**HOUSE RULES** — This functionality allows files to be uploaded to your server, so it should be
	used with caution. This feature must be manually enabled by the server owner by setting the
	`enable_upload` configuration variable to `true`.
</Callout>

Uploads let your shelf grow from the other direction — readers can add their own books straight into your longbox instead of you doing all the filing. It's opt-in and off by default, so nothing arrives until you decide to allow it.

## Enabling upload

Uploading is **disabled by default**, and is gated behind the `enable_upload` configuration variable (`true` enables uploading). This variable can be set by modifying your `Longbox.toml` file to set `enable_upload = true` or by setting the environment variable `LONGBOX_ENABLE_UPLOAD=true`.
...
The server limits the maximum upload size based on your configuration, you can change `max_file_upload_size` in `Longbox.toml` or the `LONGBOX_MAX_FILE_UPLOAD_SIZE` environment variable to set the maximum allowed upload size **in bytes**.

## » Filing new books onto the shelf (how it works)
...
<Accordions type="multiple">
  ...
  <Accordion title="404 Not found" id="404-not-found">
      This error may indicate that uploading has not been enabled for your server. Double check that you have `enable_upload = true` in your `Longbox.toml` or `LONGBOX_ENABLE_UPLOAD = true` set in your environment.
  </Accordion>

  <Accordion title="413 Payload too large" id="413-payload-too-large">
      The uploaded files exceeded your configured maximum upload size. Consider increasing the value of `max_file_upload_size` in `Longbox.toml` or changing the value of the `LONGBOX_MAX_FILE_UPLOAD_SIZE` environment variable.
  </Accordion>
</Accordions>
...
Longbox attempts to validate uploads in order to protect the host system from misuse of upload functionality, but it should be used with care. The following section describes the validation Longbox applies to incoming upload requests.
...
<Callout type="idea">
	**FROM THE LONGBOX** — If you have any questions or encounter issues, please open an
	[issue](https://github.com/SaintedRogue/longbox/issues/new/choose) on GitHub.
</Callout>
