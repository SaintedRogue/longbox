...
Uploading is **disabled by default**, and is gated behind the `enable_upload` configuration variable (`true` enables uploading). This variable can be set by modifying your `Longbox.toml` file to set `enable_upload = true` or by setting the environment variable `STUMP_ENABLE_UPLOAD=true`.
...
The server limits the maximum upload size based on your configuration, you can change `max_file_upload_size` in `Longbox.toml` or the `STUMP_MAX_FILE_UPLOAD_SIZE` environment variable to set the maximum allowed upload size **in bytes**.
...
<Accordions type="multiple">
  ...
  <Accordion title="404 Not found" id="404-not-found">
      This error may indicate that uploading has not been enabled for your server. Double check that you have `enable_upload = true` in your `Longbox.toml` or `STUMP_ENABLE_UPLOAD = true` set in your environment.
  </Accordion>

  <Accordion title="413 Payload too large" id="413-payload-too-large">
      The uploaded files exceeded your configured maximum upload size. Consider increasing the value of `max_file_upload_size` in `Longbox.toml` or changing the value of the `STUMP_MAX_FILE_UPLOAD_SIZE` environment variable.
  </Accordion>
</Accordions>
