...
<Callout title="Permission required">
	**HOUSE RULES** — This functionality is gated behind the `ACCESS_SMART_LIST` user permission. To
	learn more about permissions, see the [permissions](/docs/guides/access-control/permissions) guide.
</Callout>

A **Smart List** is a shelf that sorts itself: point it at a set of configured filters and it keeps curating which books belong, no manual upkeep required. This lets you quickly organize a collection of books based on a set of criteria without having to manually manage the list.
...
<Callout title="Work in progress">
	**PULL LIST** — The `shared` visibility option is not yet fully implemented. It functions as
	`private` for now, but will be updated in the future
</Callout>
...
<Callout title="Work in progress" type="info">
	**PULL LIST** — The look and feel of the filter configuration may change in the future to better
	support different languages and complex values. If you have any feedback, please let us know!
</Callout>
...
> You want to create a list of all books in your library that are considered "Science Fiction". However, you don't want to include any books from your Magazine library and you don't want anything Star Wars related
