...
A book club isn't just a shared shelf — it's a place to talk about what's on it.

<Callout title="Work in progress" type="idea">
	**PULL LIST** — This aspect isn't fully thought out yet. Please share ideas by opening an
	[issue](https://github.com/SaintedRogue/longbox/issues/new/choose) if you have any!
</Callout>
...
While Longbox is notably **not** a chat application, we will try to make these as functional as possible while still keeping the scope reasonable. Some features that we think would be nice to have:
...
<Callout type="warning" title="Not a secure messaging platform">
    **MINT CONDITION** — You own your data, so hypothetically there isn't a third party snooping on your messages, but we want to be very clear that **chat features are not and will not be designed to be a secure messaging platform.**
</Callout>
