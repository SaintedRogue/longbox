...
Longbox has a flexible layout system that allows you to customize the layout of the app to your liking. There are more options planned, but for now the two main options are navigation placement
...
<Callout>
	Longbox tracks the timestamp for each library you visit, but only exposes the most recent one in the
