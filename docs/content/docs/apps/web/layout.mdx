...
Longbox's shelf is yours to arrange — how you browse it, and how it's laid out, are both up to you. There are more options planned, but for now the two main levers are navigation placement and arrangement.
...
<Callout>
	**FROM THE LONGBOX** — Longbox tracks the timestamp for each library you visit, but only exposes
	the most recent one in the UI. If you feel like this feature could be improved to utilize a
	history of visited libraries, please let us know!
</Callout>
...
<Callout>
	**FROM THE LONGBOX** — If you have disabled the secondary, settings sidebar, you will either need
	to access the select box towards the top labeled `Section` and select `Appearance` or use the
	topbar settings menu to navigate to the appearance settings
</Callout>
</Step>
...
In the future, we would love to have the time to implement more options for the layout system. Some ideas include:
