...
impl MetadataProvider for LocgClient {
	...
	async fn fetch_media_metadata(
		...
		Ok(ExternalMediaMetadata {
			...
			genres: None,
			tags: None,
			// Collected editions list an ISBN where single issues list a UPC. The UPC has
			// nowhere to go; the ISBN does, and a trade-heavy library is mostly collected
			// editions, so it is worth carrying.
			isbn: page.isbn.clone(),
			isbn_13: page.isbn.clone().filter(|isbn| isbn.len() == 13),
